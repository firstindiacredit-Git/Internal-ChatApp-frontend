...
const UserDashboard = () => {
  ...
  const [showSettingsDropdown, setShowSettingsDropdown] = useState(false) // For settings dropdown
  ...
  useEffect(() => {
    const handleServiceWorkerMessage = (event) => {
      console.log('🔔 UserDashboard - Received Service Worker Message:', event.data)
      const message = event.data
      
      if (!message || !message.type) {
        console.warn('UserDashboard - Invalid message received from service worker')
        return
      }
      
      if (message.type === 'notification-clicked' || message.type === 'answer-call') {
        console.log('🔔 UserDashboard - Processing notification click/answer:', message.type)
        const notificationData = message.data
        
        if (!notificationData) {
          console.warn('UserDashboard - No notification data found')
          return
        }
        
        // Handle incoming call notifications
        if (notificationData.type === 'incoming-call') {
          console.log('📞 UserDashboard - Showing incoming call UI from notification click')
          console.log('UserDashboard - Call data:', notificationData)
          ...
          console.log('UserDashboard - Setting call UI with data:', callDataObj)
          setIsIncomingCall(true)
        ...
        // Handle incoming group call notifications
          ...
          console.log('📞 UserDashboard - Showing incoming group call UI from notification click')
          console.log('UserDashboard - Group call data:', notificationData)
          ...
          console.log('UserDashboard - Setting group call UI with data:', groupCallDataObj)
          setIsIncomingGroupCall(true)
    ...
    console.log('📡 UserDashboard - Setting up service worker message listener')
    
    // Add event listener for service worker messages
    if (navigator.serviceWorker) {
      navigator.serviceWorker.addEventListener('message', handleServiceWorkerMessage)
      console.log('✅ UserDashboard - Service worker message listener attached')
    } else {
      console.warn('⚠️ UserDashboard - Service worker not available')
    }
    ...
    if (notificationDataParam) {
      console.log('📎 UserDashboard - Found notification data in URL')
      try {
        const decoded = atob(notificationDataParam)
        const message = JSON.parse(decoded)
        console.log('🔔 UserDashboard - Notification data from URL:', message)
        ...
        console.error('UserDashboard - Error parsing notification data from URL:', error)
      }
    }
    
    // Cleanup
    return () => {
      console.log('🧹 UserDashboard - Removing service worker message listener')
      if (navigator.serviceWorker) {
  ...
  // Close message context menu on scroll or escape key
  useEffect(() => {
    const handleScroll = () => setMessageContextMenu(null)
    const handleKeyDown = (e) => {
  ...
  const allChats = [
    ...
    // If both pinned or both unpinned, sort by most recent message timestamp
                              ...
                              <div className="mb-2">
                                {message.messageType === 'image' ? (
                                  <div className="relative group" onMouseDown={() => startLongPress(message)} onMouseUp={cancelLongPress} onMouseLeave={cancelLongPress} onTouchStart={() => startLongPress(message)} onTouchEnd={cancelLongPress}>
                                    ...
                                    {/* Dropdown menu */}
                                    {messageDropdown === getMessageId(message) && (
                                      <div className="message-dropdown-container absolute bottom-full right-2 mb-2 z-20 bg-white rounded-lg shadow-lg border border-gray-200 py-1 min-w-[120px]">
                                        <button
                                          onClick={() => handleDropdownAction('copy', message)}
                                          className="w-full px-3 py-2 text-left text-sm text-gray-700 hover:bg-gray-100 flex items-center gap-2"
                                        >
